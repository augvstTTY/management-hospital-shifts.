...
def load_data():
    ...
    # Default staff dataset (formatted across multiple lines to satisfy line-length rules)
    default_staff = [
        {
            "Código": "ROS-101",
            ...
            "Turnos_nocturnos_mes": 2,
            "Festivos_mes": 1,
            "Hospital": "Hospital Rosales",
        },
        {
            "Código": "ROS-102",
        ...
        {
            "Código": "ZAC-303",
            "Nombre": "Marta Elizabeth Contreras",
            "Antigüedad": 4,
            "Especialidad": "Pediatría",
            "Turnos_nocturnos_mes": 1,
            "Festivos_mes": 0,
        ...
        },
        {
            ...
            "Turnos_nocturnos_mes": 3,
            "Festivos_mes": 2,
            "Hospital": "Hospital Nacional de Niños",
        },
        {
            "Código": "HNN-402",
        ...
        {
            "Código": "HNN-403",
            "Nombre": "Carlos Alfredo Portillo",
            "Antigüedad": 5,
            "Especialidad": "Medicina Interna",
            "Turnos_nocturnos_mes": 0,
            "Festivos_mes": 1,
        ...
        for shift in ["Noche (23:00-7:00)", "Tarde (15:00-23:00)", "Mañana (7:00-15:00)"]:
            for person in available:
                ...
                if (
                    "Mañana" in shift
                    and day > 0
                    and shifts[-1]["Noche (23:00-7:00)"]
                    and person["Nombre"] in shifts[-1]["Noche (23:00-7:00)"]
                ):
                    continue
        ...
        shifts.append(
            {
                "Fecha": date_str,
                "Día": date.strftime("%A"),
                "Festivo": f"✓ {holidays.get(date_str, '')}" if is_holiday else "",
...
with st.sidebar:
    ...
    st.metric(
        "Available Staff",
        len([p for p in staff if (selected_hospital == "All" or p["Hospital"] == selected_hospital)]),
    )
...
if st.session_state.get("edit_staff", False):
    with st.expander("📝 Staff Editor", expanded=True):
        st.markdown(
            '<div class="warning-box">⚠ Edit carefully. Changes will be saved permanently.</div>',
            unsafe_allow_html=True,
        )
        ...
        with col1:
            if st.button("💾 Save Changes", key="save_staff"):
                ...
                for i, emp in enumerate(staff_updated, 1):
                    ...
                    if not isinstance(emp.get("Antigüedad"), int) or emp["Antigüedad"] < 0:
                        errors.append(f"Invalid seniority for {emp.get('Nombre', 'employee')}")
                    if not isinstance(emp.get("Turnos_nocturnos_mes"), int) or emp["Turnos_nocturnos_mes"] < 0:
                        errors.append(f"Invalid night shifts for {emp.get('Nombre', 'employee')}")
                    if not isinstance(emp.get("Festivos_mes"), int) or emp["Festivos_mes"] < 0:
                ...
                if errors:
                    ...
                    if save_staff(staff):
                        st.markdown(
                            '<div class="success-box">Staff data successfully updated!</div>',
                            unsafe_allow_html=True,
                        )
            ...
            if st.button("💾 Save Holidays", key="save_holidays"):
                ...
                for i, row in edited_holidays.iterrows():
                    ...
                    try:
                        ...
                        errors.append(
                            f"Line {i + 1}: Incorrect date format '{date_str}'. Use DD/MM/YYYY",
                        )
                        continue
                    ...
                    new_holidays[date_str] = str(name).strip()
                ...
                if errors:
                    ...
                    st.markdown(
                        '<div class="success-box">Holiday data successfully updated!</div>',
                        unsafe_allow_html=True,
                    )
...
if specialty_filter != "All":
    staff_specialty = [
        f"{p['Código']} - {p['Nombre']}" for p in staff if p["Especialidad"] == specialty_filter
    ]
    mask = (
...
with tab1:
    title_hospital = selected_hospital if selected_hospital != "All" else "All hospitals"
    st.markdown(f"### Shifts - {title_hospital}")
    ...
    st.dataframe(
        df_display.style.applymap(
            lambda x: "background-color: #FFF2CC" if "✓" in str(x) else "",
            subset=["Festivo"],
        ).applymap(
            lambda x: "background-color: #E6F3FF" if "Noche" in str(x) else "",
            subset=["Noche (23:00-7:00)"],
        ),
        height=600,
        use_container_width=True,
    )
...
with tab2:
    ...
    with col2:
        ...
        specialty_data = pd.DataFrame(
            [{"Specialty": p["Especialidad"], "Count": 1} for p in staff]
        )
    ...
    summary = pd.DataFrame(
        {
            ...
            "Involved Staff": [len(set(involved))],
        }
    ).T
